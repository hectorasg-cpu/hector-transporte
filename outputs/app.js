window.__appVersion = 'tipo-pedido-alertas-20260703a';
const state = {
  clients: [
...
function loadTypeSummary(order) { const type = order.loadType || 'Paletizado'; const needsCounts = type === 'Suelto' || type === 'Mixto'; const pallets = Number(order.palletCount || 0); const cholguan = Number(order.cholguanCount || 0); if (!needsCounts) return type; return type + ' - ' + pallets + ' pallets - ' + cholguan + ' cholguan'; }
...
const schemas = {
  ...
  orders: { title: 'Pedido', prefix: 'o', fields: [['code', 'Codigo', 'text'], ['origin', 'Origen', 'text'], ['date', 'Fecha', 'date'], ['truckId', 'Camion', 'trucks'], ['driverId', 'Conductor', 'drivers'], ['status', 'Estado', 'select', ['Pendiente', 'Programado', 'En ruta', 'Terminado']], ['loadType', 'Tipo de pedido', 'select', ['Suelto', 'Paletizado', 'Mixto']], ['palletCount', 'Cantidad pallets', 'number'], ['cholguanCount', 'Cantidad cholguan', 'number']] }
};
let editing = null;

function defaultValue(collection, key) { if (collection === 'orders' && key === 'code') return 'PED-' + (1000 + state.orders.length + 1); if (key === 'date') return '2026-07-02'; if (key === 'status') return collection === 'orders' ? 'Pendiente' : 'Disponible'; if (key === 'loadType') return 'Paletizado'; if (key === 'palletCount' || key === 'cholguanCount') return '0'; if (key === 'truckId') return state.trucks[0]?.id || ''; if (key === 'driverId') return state.drivers[0]?.id || ''; return ''; }
...
function updateLoadCountFields() { const type = $('#loadType')?.value || 'Paletizado'; const showCounts = type === 'Suelto' || type === 'Mixto'; ['palletCount', 'cholguanCount'].forEach((id) => { const input = $('#' + id); if (!input) return; const field = input.closest('.field'); if (field) field.hidden = !showCounts; if (!showCounts) input.value = '0'; }); }
...
on('#editorFields', 'change', (event) => { if (event.target?.id === 'loadType') updateLoadCountFields(); });
