...
const state = {
  ...
  products: [
    { id: "sku1", sku: "VS-15-TINTO-ROSCA", cap: "Rosca", capColor: "Negra", volume: "1.5 L", wineType: "Tinto", brand: "Vina Saavedra" },
    { id: "sku2", sku: "VS-15-BLANCO-ROSCA", cap: "Rosca", capColor: "Blanca", volume: "1.5 L", wineType: "Blanco", brand: "Vina Saavedra" }
  ],
  trash: [],
  auditLog: [],
  orders: [
...
function normalizeSearchText(value) { return String(value || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim(); }
function matchesSearch(text, query) { const normalizedQuery = normalizeSearchText(query); return !normalizedQuery || normalizeSearchText(text).includes(normalizedQuery); }
function setSearchCount(selector, shown, total) { const target = $(selector); if (!target) return; target.textContent = total ? shown + ' de ' + total : '0 registros'; }
function emptyRow(colspan, title, detail) { return '<tr><td colspan="' + colspan + '"><div class="empty-row"><strong>' + title + '</strong><span>' + detail + '</span></div></td></tr>'; }
function productLabel(product = {}) { return [product.sku, product.brand, product.wineType, product.volume, product.cap, product.capColor ? 'Tapa ' + product.capColor : ''].filter(Boolean).join(' - ') || product.id || 'Producto'; }
function integrationSku(product = {}) { return product.integrationSku || product.integrationCode || product.normalizedSku || ''; }
function normalizedProductLines(stop = {}) { const lines = Array.isArray(stop.productLines) ? stop.productLines : []; const normalized = lines.map((line) => ({ productId: line.productId || '', quantity: line.quantity ?? line.qty ?? '0', notes: line.notes || '' })).filter((line) => line.productId || Number(line.quantity || 0)); if (normalized.length) return normalized; const legacy = []; if (Number(stop.bottles || 0)) legacy.push({ productId: '', quantity: stop.bottles, notes: 'Botellas registradas antes de usar SKU' }); if (Number(stop.drums || 0)) legacy.push({ productId: '', quantity: stop.drums, notes: 'Bidones registrados antes de usar SKU' }); return legacy; }
function productLinesSummary(stop = {}) { const lines = normalizedProductLines(stop); return lines.length ? lines.map((line) => { const product = byId('products', line.productId); const label = product ? productLabel(product) : (line.notes || 'Producto anterior'); return Number(line.quantity || 0) + ' x ' + label; }).join(' | ') : 'Sin productos'; }
...
function stopsSummary(order) { const totals = stopTotals(order); return '<div class="route-list">' + operationalStops(order).map((stop, index) => { const client = byId('clients', stop.clientId); const payment = stop.collectPayment ? ' · Cobro ' + money(stop.collectionAmount) + ' (' + stop.paymentStatus + ')' : ''; return '<span><strong>' + (index + 1) + '. ' + (client?.name || 'Sin cliente') + '</strong> a ' + (stop.destination || 'Sin destino') + '<em> · ' + stopLoadSummary(stop) + ' · ' + Number(stop.kilos || 0) + ' kg · Productos: ' + productLinesSummary(stop) + payment + '</em></span>'; }).join('') + '<span class="totals-line">Total: ' + totals.kilos + ' kg · ' + totals.products + ' unidades de productos · ' + money(totals.collection) + ' por cobrar</span></div>'; }
...
function renderOrders() { const query = $('#orderSearch')?.value || ''; const rows = state.orders.filter((order) => { const truck = byId('trucks', order.truckId), driver = byId('drivers', order.driverId); const clients = operationalStops(order).map((stop) => byId('clients', stop.clientId)?.name || '').join(' '); const helpers = orderHelpers(order).map((helper) => helper.name).join(' '); return matchesSearch([order.code, order.origin, order.date, formatDate(order.date), order.status, orderDestinations(order), loadTypeSummary(order), orderStops(order).map(productLinesSummary).join(' '), clients, truck?.plate, truck?.model, driver?.name, helpers, settlementStatus(order)].join(' '), query); }); setSearchCount('#orderSearchCount', rows.length, state.orders.length); $('#ordersTable').innerHTML = rows.length ? rows.map((order) => { const truck = byId('trucks', order.truckId), driver = byId('drivers', order.driverId); return '<tr><td>' + order.code + '</td><td>' + stopsSummary(order) + '</td><td>' + order.origin + '</td><td>' + formatDate(order.date) + '</td><td>' + loadTypeSummary(order) + '</td><td>' + (truck?.plate || 'Sin camion') + '</td><td>' + (driver?.name || 'Sin conductor') + '</td><td>' + helpersSummary(order) + '</td><td>' + badge(order.status) + '</td><td>' + settlementSummary(order) + '</td><td><div class="actions"><button class="text-button" data-edit="orders" data-id="' + order.id + '">Editar</button><button class="text-button" data-delete="orders" data-id="' + order.id + '">Eliminar</button></div></td></tr>'; }).join('') : emptyRow(11, 'Sin pedidos encontrados', 'Prueba con otro codigo, cliente, destino o fecha.'); }
function ensureProductsHeader() { const table = $('#productsTable'); const head = table?.closest('table')?.querySelector('thead tr'); if (!head) return; head.innerHTML = '<th>SKU</th><th>Codigo integracion</th><th>Tapa</th><th>Color de tapa</th><th>Volumen</th><th>Tipo de vino</th><th>Marca</th><th></th>'; }
function renderProducts() { const table = $('#productsTable'); if (!table) return; ensureProductsHeader(); table.innerHTML = state.products.length ? state.products.map((product) => '<tr><td>' + product.sku + '</td><td>' + integrationSku(product) + '</td><td>' + product.cap + '</td><td>' + (product.capColor || '') + '</td><td>' + product.volume + '</td><td>' + product.wineType + '</td><td>' + product.brand + '</td><td><div class="actions"><button class="text-button" data-edit="products" data-id="' + product.id + '">Editar</button><button class="text-button" data-delete="products" data-id="' + product.id + '">Eliminar</button></div></td></tr>').join('') : emptyRow(8, 'Sin productos creados', 'Agrega SKU para armar pedidos con productos.'); }
function renderClients() { const query = $('#clientSearch')?.value || ''; const rows = state.clients.filter((client) => matchesSearch([client.name, client.contact, client.phone, client.city].join(' '), query)); setSearchCount('#clientSearchCount', rows.length, state.clients.length); $('#clientsTable').innerHTML = rows.length ? rows.map((client) => '<tr><td>' + client.name + '</td><td>' + client.contact + '</td><td>' + client.phone + '</td><td>' + client.city + '</td><td><div class="actions"><button class="text-button" data-edit="clients" data-id="' + client.id + '">Editar</button><button class="text-button" data-delete="clients" data-id="' + client.id + '">Eliminar</button></div></td></tr>').join('') : emptyRow(5, 'Sin clientes encontrados', 'Prueba con otro nombre, telefono, contacto o ciudad.'); }
...
function renderAll() { renderDashboard(); renderOrders(); renderClients(); renderProducts(); renderTrucks(); renderTractorTrailers(); renderDrivers(); renderHelpers(); renderCalendar(); renderHistory(); }

const schemas = {
  clients: { title: 'Cliente', prefix: 'c', fields: [['name', 'Nombre', 'text'], ['contact', 'Contacto', 'text'], ['phone', 'Telefono', 'tel'], ['city', 'Ciudad', 'text']] },
  products: { title: 'Producto', prefix: 'sku', fields: [['sku', 'SKU/Codigo', 'text'], ['integrationSku', 'Codigo integracion', 'text'], ['cap', 'Tapa', 'text'], ['capColor', 'Color de tapa', 'text'], ['volume', 'Volumen', 'text'], ['wineType', 'Tipo de vino', 'text'], ['brand', 'Marca', 'text']] },
...
function readProductLines(row) { return Array.from(row.querySelectorAll('.product-line')).map((line) => ({ productId: line.querySelector('[name="lineProduct"]')?.value || '', quantity: line.querySelector('[name="lineQuantity"]')?.value || '0', notes: line.querySelector('[name="lineNotes"]')?.value.trim() || '' })).filter((line) => line.productId && Number(line.quantity || 0) > 0); }
function readStops() { return Array.from(document.querySelectorAll('.stop-row')).map((row) => { const collectPayment = row.querySelector('[name="stopCollectPayment"]').value === 'yes'; const loadType = row.querySelector('[name="stopLoadType"]')?.value || 'Paletizado'; return { clientId: row.querySelector('[name="stopClient"]').value, destination: row.querySelector('[name="stopDestination"]').value.trim(), loadType, kilos: row.querySelector('[name="stopKilos"]')?.value || '0', palletCount: loadType === 'Paletizado' ? '0' : (row.querySelector('[name="stopPalletCount"]')?.value || '0'), cholguanCount: loadType === 'Paletizado' ? '0' : (row.querySelector('[name="stopCholguanCount"]')?.value || '0'), productLines: readProductLines(row), bottles: '', drums: '', collectPayment, collectionAmount: collectPayment ? (row.querySelector('[name="stopCollectionAmount"]').value || '0') : '', paymentStatus: collectPayment ? row.querySelector('[name="stopPaymentStatus"]').value : 'No aplica', notes: row.querySelector('[name="stopNotes"]').value.trim() }; }).filter((stop) => stop.clientId && stop.destination); }
...
document.addEventListener('click', (event) => { const saveButton = event.target.closest('#saveEditor'); if (saveButton) { saveEditor(event); return; } const closeEditor = event.target.closest('[data-close-editor]'); if (closeEditor) { $('#editor').close(); return; } const tab = event.target.closest('[data-view]'), viewLink = event.target.closest('[data-view-link]'), edit = event.target.closest('[data-edit]'), remove = event.target.closest('[data-delete]'), restoreTrash = event.target.closest('[data-restore-trash]'), purgeTrash = event.target.closest('[data-purge-trash]'), restoreHistory = event.target.closest('[data-restore-history]'), addStop = event.target.closest('[data-add-stop]'), removeStop = event.target.closest('[data-remove-stop]'), addProductLine = event.target.closest('[data-add-product-line]'), removeProductLine = event.target.closest('[data-remove-product-line]'); if (tab) setView(tab.dataset.view); if (viewLink) setView(viewLink.dataset.viewLink); if (edit) openEditor(edit.dataset.edit, edit.dataset.id); if (remove) deleteItem(remove.dataset.delete, remove.dataset.id); if (restoreTrash) restoreTrashItem(restoreTrash.dataset.restoreTrash); if (purgeTrash) purgeTrashItem(purgeTrash.dataset.purgeTrash); if (restoreHistory) restoreHistoryEntry(restoreHistory.dataset.restoreHistory); if (addStop) { $('#stopsRows').insertAdjacentHTML('beforeend', stopRow({ clientId: state.clients[0]?.id || '', destination: '', loadType: 'Paletizado', kilos: '0', palletCount: '0', cholguanCount: '0', productLines: [{ productId: state.products[0]?.id || '', quantity: '0', notes: '' }], drums: '', bottles: '', collectPayment: false, collectionAmount: '', paymentStatus: 'No aplica', notes: '' }, $('.stop-row').length)); rerenderStopNumbers(); updateLoadCountFields(); } if (removeStop) { const rows = $('.stop-row'); if (rows.length > 1) { removeStop.closest('.stop-row').remove(); rerenderStopNumbers(); updateStopKilosTotal(); } } if (addProductLine) { const box = addProductLine.closest('.stop-products'); box.querySelector('.product-lines').insertAdjacentHTML('beforeend', productLineRow({ productId: state.products[0]?.id || '', quantity: '0', notes: '' })); } if (removeProductLine) { const lines = Array.from(removeProductLine.closest('.product-lines').querySelectorAll('.product-line')); if (lines.length > 1) removeProductLine.closest('.product-line').remove(); } const moveStop = event.target.closest('[data-move-stop]'); if (moveStop) { const row = moveStop.closest('.stop-row'); if (moveStop.dataset.moveStop === 'up' && row.previousElementSibling) row.parentNode.insertBefore(row, row.previousElementSibling); if (moveStop.dataset.moveStop === 'down' && row.nextElementSibling) row.parentNode.insertBefore(row.nextElementSibling, row); rerenderStopNumbers(); } });
...
on('#newProduct', 'click', () => openEditor('products'));
