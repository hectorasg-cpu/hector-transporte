window.__appVersion = 'tipo-por-parada-20260703c';
const state = {
  clients: [
...
function stopsSummary(order) { const totals = stopTotals(order); return '<div class="route-list">' + operationalStops(order).map((stop, index) => { const client = byId('clients', stop.clientId); const payment = stop.collectPayment ? ' · Cobro ' + money(stop.collectionAmount) + ' (' + stop.paymentStatus + ')' : ''; return '<span><strong>' + (index + 1) + '. ' + (client?.name || 'Sin cliente') + '</strong> a ' + (stop.destination || 'Sin destino') + '<em> · ' + stopLoadSummary(stop) + ' · ' + Number(stop.drums || 0) + ' bidones · ' + Number(stop.bottles || 0) + ' botellas' + payment + '</em></span>'; }).join('') + '<span class="totals-line">Total: ' + totals.drums + ' bidones · ' + totals.bottles + ' botellas · ' + money(totals.collection) + ' por cobrar</span></div>'; }
...
on('#editorFields', 'change', (event) => { if (event.target?.name === 'stopLoadType') updateLoadCountFields(); });
