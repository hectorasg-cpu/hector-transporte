window.__appVersion = 'productos-sku-20260711';
const state = {
  clients: [
  ...
  products: [
    { id: "sku1", sku: "VS-15-TINTO-ROSCA", cap: "Rosca", volume: "1.5 L", wineType: "Tinto", brand: "Vina Saavedra" },
    { id: "sku2", sku: "VS-15-BLANCO-ROSCA", cap: "Rosca", volume: "1.5 L", wineType: "Blanco", brand: "Vina Saavedra" }
  ],
  trash: [],
  auditLog: [],
  orders: [
...
function productLabel(product = {}) { return [product.sku, product.brand, product.wineType, product.volume, product.cap].filter(Boolean).join(' - ') || product.id || 'Producto'; }
...
function renderProducts() { const table = $('#productsTable'); if (!table) return; table.innerHTML = state.products.length ? state.products.map((product) => '<tr><td>' + product.sku + '</td><td>' + product.cap + '</td><td>' + product.volume + '</td><td>' + product.wineType + '</td><td>' + product.brand + '</td><td><div class="actions"><button class="text-button" data-edit="products" data-id="' + product.id + '">Editar</button><button class="text-button" data-delete="products" data-id="' + product.id + '">Eliminar</button></div></td></tr>').join('') : emptyRow(6, 'Sin productos creados', 'Agrega SKU para armar pedidos con productos.'); }
...
const schemas = {
  ...
  products: { title: 'Producto', prefix: 'sku', fields: [['sku', 'SKU/Codigo', 'text'], ['cap', 'Tapa', 'text'], ['volume', 'Volumen', 'text'], ['wineType', 'Tipo de vino', 'text'], ['brand', 'Marca', 'text']] },
