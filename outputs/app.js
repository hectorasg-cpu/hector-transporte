window.__appVersion = 'color-tapa-20260713b';
const state = {
  clients: [
...
function ensureProductsHeader() { const table = $('#productsTable'); const head = table?.closest('table')?.querySelector('thead tr'); if (!head) return; head.innerHTML = '<th>SKU</th><th>Tapa</th><th>Color de tapa</th><th>Volumen</th><th>Tipo de vino</th><th>Marca</th><th></th>'; }
function renderProducts() { const table = $('#productsTable'); if (!table) return; ensureProductsHeader(); table.innerHTML = state.products.length ? state.products.map((product) => '<tr><td>' + product.sku + '</td><td>' + product.cap + '</td><td>' + (product.capColor || '') + '</td><td>' + product.volume + '</td><td>' + product.wineType + '</td><td>' + product.brand + '</td><td><div class="actions"><button class="text-button" data-edit="products" data-id="' + product.id + '">Editar</button><button class="text-button" data-delete="products" data-id="' + product.id + '">Eliminar</button></div></td></tr>').join('') : emptyRow(7, 'Sin productos creados', 'Agrega SKU para armar pedidos con productos.'); }
...
const schemas = {
  ...
  products: { title: 'Producto', prefix: 'sku', fields: [['sku', 'SKU/Codigo', 'text'], ['cap', 'Tapa', 'text'], ['capColor', 'Color de tapa', 'text'], ['volume', 'Volumen', 'text'], ['wineType', 'Tipo de vino', 'text'], ['brand', 'Marca', 'text']] },
