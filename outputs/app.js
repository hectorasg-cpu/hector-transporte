window.__appVersion = 'color-tapa-20260713';
const state = {
  clients: [
...
function renderProducts() { const table = $('#productsTable'); if (!table) return; table.innerHTML = state.products.length ? state.products.map((product) => '<tr><td>' + product.sku + '</td><td>' + product.cap + '</td><td>' + (product.capColor || '') + '</td><td>' + product.volume + '</td><td>' + product.wineType + '</td><td>' + product.brand + '</td><td><div class="actions"><button class="text-button" data-edit="products" data-id="' + product.id + '">Editar</button><button class="text-button" data-delete="products" data-id="' + product.id + '">Eliminar</button></div></td></tr>').join('') : emptyRow(7, 'Sin productos creados', 'Agrega SKU para armar pedidos con productos.'); }
